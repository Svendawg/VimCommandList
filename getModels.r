...
#Remove those with non-finite values
	...
	errorVector <- getErrorVector(X, Y);
	plot(errorVector);
}
...
getErrorVector <- function(X, Y){
	nRows <- dim(X)[1]
print(length(nRows));
print("length(nRows) above");
	error <- rep(0, nRows);
	for(i in 1:nRows)
     {
		print(i);
		error <- error + getLassoError(X, Y, i);
	}
	error <- error/nRows
}

## the jth element of the returned vector is the squared error for the ith element of Y and
## the jth element of lambda
